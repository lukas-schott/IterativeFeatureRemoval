...
def main():
    ...
    for loop in range(config.n_loops):
        ...
        model.eval()
        adversaries = [att.get_attack(model, 'l2', config, attack) for attack in config.attacks_names]
        ...
        if config.training_mode == 'overwrite':
            ...
            for name in ['train', 'test']:
                ...
                adversary = att.get_attack(model, config.lp_metric, config, config.attack_for_new_dataset,
